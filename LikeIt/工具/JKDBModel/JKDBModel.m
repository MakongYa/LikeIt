...
+ (NSDictionary *)getPropertys
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        if ([propertyType rangeOfString: @"NSNumber"].location != NSNotFound) {
            [proTypes addObject:SQLINTEGER];
        }
        else if ([propertyType hasPrefix:@"T@"]) {
            [proTypes addObject:SQLTEXT];
        } else if ([propertyType hasPrefix:@"Ti"]||[propertyType hasPrefix:@"TI"]||[propertyType hasPrefix:@"Ts"]||[propertyType hasPrefix:@"TS"]||[propertyType hasPrefix:@"TB"] ||[propertyType hasPrefix:@"L"]) {
            [proTypes addObject:SQLINTEGER];
        } else {
...
- (BOOL)update:(NSString *)criteria
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        //删除最后那个逗号
        [keyString deleteCharactersInRange:NSMakeRange(keyString.length - 1, 1)];
        NSString *sql = [NSString stringWithFormat:@"UPDATE %@ SET %@ %@;", tableName, keyString, criteria];
        [updateValues addObject:primaryValue];
        res = [db executeUpdate:sql withArgumentsInArray:updateValues];
        NSLog(res?@"更新成功":@"更新失败");
    }];
